...
/*
 * Telemetry dashboard HTML/CSS/JS
 * Stored in flash (PROGMEM) to avoid consuming SRAM.
 *
 * Packet format (15 CSV fields):
 *   0  mode          (0=MAN, 1=AUTO)
 *   1  ctrl          (0=no, 1=yes)
 *   2  irL           (0/1)
 *   3  irC           (0/1)
 ...
 *   5  rpmL*10       (integer)
 *   6  rpmR*10       (integer)
 *   7  distance*10   (integer, mm*10)
 *   8  heading*10    (integer, deg*10, signed)
 *   9  servoDeg      (integer)
 *  10  cmdLeft       (integer, -1023..1023)
 *  11  cmdRight      (integer, -1023..1023)
 *  12  autoBase      (integer, 0..1023)
 *  13  autoTurnSoft  (integer, 0..1023)
 *  14  assistActive  (0/1)
 */
...
<style>
/* ── Design: Industrial CRT Terminal ─────────────────────────────── */
:root {
  --bg:         #0a0c0e;
  --panel:      #0f1215;
  --border:     #1e2530;
...
function setMotorBar(fillId, valId, cmd) {
  ...
  if (!fill || !val) return;
  // Track is split at centre (50% = zero). Each half = 100% of available space.
...
function connect() {
  ...
  ws.onmessage = (ev) => {
    ...
    const f = s.split(',');
    if (f.length !== 15) return;   // firmware sends 15 fields
